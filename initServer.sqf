...
[] spawn 
{
    ...
    // Load Occupation Config
    call compile preprocessFileLineNumbers "\x\addons\a3_exile_occupation\config.sqf";
    
    if (isNil "SC_CompiledOkay") exitWith { 
        diag_log format ["[OCCUPATION]:: Occupation failed to read config.sqf, check for typos (time: %1)", time]; 
    };

    // Load Logging Function
    SC_fnc_log = compile preprocessFileLineNumbers "\x\addons\a3_exile_occupation\scripts\occupationLog.sqf";

    // Start Occupation System
    [] execVM "\x\addons\a3_exile_occupation\scripts\startOccupation.sqf";

    // Start AI Roadblocks if enabled
    if (SC_occupyRoadblocks) then {
        [] execVM "\x\addons\a3_exile_occupation\scripts\occupationRoadblock_rotation.sqf";
    };

    // Log completion
    diag_log format ["[OCCUPATION MOD]:: Occupation %2 Fully Loaded at %1", time, SC_occupationVersion];
